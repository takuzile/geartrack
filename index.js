...
const CLIENT_ID     = '256853';
const CLIENT_SECRET = 'a6b802bd711da2ce0516955711df2cdab521e7bc';
const PORT          = 3000;
...
app.use(session({
  secret: 'geartrack_session_secret_2024',
  resave: false,
  saveUninitialized: false,
  cookie: { maxAge: 7 * 24 * 60 * 60 * 1000 }, // 7日閁E}));
...
const DEFAULT_DATA = {
  ...
  products: [
    { id:1, cat:'tire',  brand:'Continental', name:'GP5000 28c',  avgKm:4100, users:1240, heavy:68, mid:24, light:8,
      reviews:[
        { user:'ヘビー (380km/朁E', km:3980, stars:5, text:'雨でもグリチE�E抜群、E000km近く使えた、E },
        { user:'ミドル (180km/朁E', km:4300, stars:4, text:'コスパ良し。ライトなめE000kmぁE��る、E },
      ],
    ...
    { id:2, cat:'tire',  brand:'Vittoria', name:'Corsa Pro TLR', avgKm:3900, users:560, heavy:55, mid:35, light:10,
      reviews:[{ user:'ヘビー (420km/朁E', km:3600, stars:4, text:'グリチE�E最高。消耗�EめE��早め、E }],
    ...
    { id:3, cat:'chain', brand:'Shimano', name:'CN-M7100',       avgKm:3200, users:980, heavy:40, mid:45, light:15,
      reviews:[{ user:'ミドル (220km/朁E', km:3100, stars:4, text:'変速スムーズ。メンチE��第で長持ち、E }],
    ...
    { id:4, cat:'chain', brand:'KMC', name:'X12-EL',             avgKm:3200, users:420, heavy:60, mid:30, light:10,
      reviews:[{ user:'ヘビー (350km/朁E', km:2900, stars:5, text:'Shimanoより耐乁E��高く感じる、E }],
    ...
    { id:5, cat:'wheel', brand:'Zipp', name:'303 S',             avgKm:18000, users:210, heavy:70, mid:25, light:5,
      reviews:[{ user:'ヘビー (400km/朁E', km:16000, stars:5, text:'剛性最高。リム面の摩耗�E少なぁE��E }],
      rivals:[{name:'Fulcrum Racing 4',km:15000}]
    },
  ],
  rewards: [
    { id:1, brand:'Continental', product:'GP5000',   target:8000, reward:'次回購入 15% OFF', code:'CONT-15-2024' },
    { id:2, brand:'Shimano',     product:'CN-M7100', target:6000, reward:'グリス プレゼンチE,  code:'SHI-GREASE' },
  ],
};

const DEFAULT_GEARS = [
  { id:1, name:'リアタイヤ',    cat:'tire',  limit:4000, used:0, start_date:null, product:'Continental GP5000' },
  { id:2, name:'フロントタイヤ', cat:'tire',  limit:4000, used:0, start_date:null, product:'Continental GP5000' },
  { id:3, name:'チェーン',      cat:'chain', limit:3000, used:0, start_date:null, product:'Shimano CN-M7100' },
  { id:4, name:'バ�EチE�EチE,    cat:'bar',   limit:5000, used:0, start_date:null, product:'' },
  { id:5, name:'ブレーキパッチE, cat:'brake', limit:5000, used:0, start_date:null, product:'' },
];

function loadDB() {
  try {
    if (fs.existsSync(DB_FILE)) return JSON.parse(fs.readFileSync(DB_FILE, 'utf8'));
  } catch(e) { console.error('DB読み込みエラー:', e.message); }
  return JSON.parse(JSON.stringify(DEFAULT_DATA));
}

function saveDB() {
  try { fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2), 'utf8'); }
  catch(e) { console.error('DB書き込みエラー:', e.message); }
}
...
// Stravaト�Eクンはメモリのみ�E��E起動時に再ログイン�E�Econst stravaTokens = {}; // { userId: tokenStore }
const stravaActivities = {}; // { userId: [...] }

// ── 認証ミドルウェア ───────────────────────────────
function requireAuth(req, res, next) {
  if (!req.session.userId) return res.status(401).json({ error: 'ログインが忁E��でぁE });
  next();
}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!email || !password || !name) return res.json({ error: '全頁E��を�E力してください' });
  if (db.users.find(u => u.email === email)) return res.json({ error: 'こ�Eメールアドレスは既に登録されてぁE��ぁE });
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (!user) return res.json({ error: 'メールアドレスまた�Eパスワードが違いまぁE });
  const ok = await bcrypt.compare(password, user.password);
  if (!ok)  return res.json({ error: 'メールアドレスまた�Eパスワードが違いまぁE });
  req.session.userId   = user.id;
  req.session.userName = user.name;
  res.json({ ok: true, name: user.name });
});
...
app.get('/api/gears', requireAuth, (req, res) => {
  res.json(db.gears[req.session.userId] || []);
});
...
app.post('/api/sync', requireAuth, async (req, res) => {
  const uid = req.session.userId;
  if (!stravaTokens[uid]) return res.json({ error: '未連携' });
  try {
...
app.listen(PORT, () => {
  ...
  console.log('登録ユーザー数:', db.users.length);
});
